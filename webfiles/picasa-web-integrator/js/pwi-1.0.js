...
var pwi_username = "mobelsnekkern"; 			//Your username at Picasa WebAlbums
var pwi_container_div = "picasa_image_container"; 	//Specifiy the id of the div in which the albums should be loaded inside your html/template/...
//setup with ONE specific album only, choose an album and take the name from the url between "#" and the last "/1"
var pwi_album_only = "";
if (typeof(picasa_album_name) != 'undefined') {
  pwi_album_only = picasa_album_name;
}
...
var pwi_album_crop = 0;					// 1 = crop to thumbsize, making them square, or 0 = keep original aspect ratio making the thumbsize the max width and height
...
var pwi_maxresults = 200; 				//pictures per page
...
pwi_labels["photo"] 	= "bilde";
pwi_labels["photos"] 	= "bilder";
pwi_labels["albums"] 	= "Back to albums";
pwi_labels["slideshow"] = "Vis slideshow";
pwi_labels["loading"] 	= "Henter bilder, vennligst vent ...";
pwi_labels["page"] 		= "Side";
pwi_labels["prev"] 		= "Forrige";
pwi_labels["next"] 		= "Nest";
pwi_labels["devider"] 	= "|";
...
function album(j) { //returns all photos in a specific album
	...
	if (pwi_show_albumdescription) {
	  //Roy: Added h4 tag for the title
		scPhotos.push("<div class='title'><h4>" + j.feed.title.$t + "</h4></div>");
...
$(document).ready(function() {
	if (pwi_username == "YOUR_USER_NAME_HERE") {
		...
		$.extend($.blockUI.defaults.overlayCSS, {
		  //Roy: Endret farge fra #000 for teksten som står på siden under lastning av bildene
			backgroundColor: '#bbb'
		});
		$.ajaxSetup({
			cache: true
